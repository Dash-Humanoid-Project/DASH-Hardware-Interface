// ===== Teensy 4 — Right arm firmware =====
// IP: 10.176.32.36 (ASSUMED — verify against the physical Teensy 4's
//     flashed static IP before first boot; see Param.h's TEENSY4_IP)
// Port: 8003
// ODrive node IDs: 14 (r_shoulder_pitch), 15 (r_shoulder_roll),
//                  16 (r_shoulder_yaw), 17 (r_elbow)
// CAN1: nodes 14, 15  |  CAN2: nodes 16, 17
//
// Unlike Teensy 3 (left arm), the local variable/array names here
// (odrv14..17, ODRV14-17) match the real CAN node_id directly — these node
// IDs are genuinely unique project-wide, no reuse/offset scheme needed.
//
// Generated from teensy3/teensy3.ino (Teensy 3, left arm) with these changes:
//   - staticIP → 10.176.32.36
//   - teensy_udp_port_listening / PC_udp_port_listening → 8003
//   - ODrive objects and user data → odrv14..17 (ODRV14-17 macros, Param.h)
//   - sys_data_ initialised with N_ODRIVE_CAN8, N_ODRIVE_CAN9
//   - CAN1 mailbox filters recomputed for node IDs 14, 15 (different real
//     node_id values than Teensy 3's, unlike Teensy 3 vs Teensy 2's reuse)
...
// ----- Network config (right arm) -----
IPAddress staticIP(10, 176, 32, 36);
IPAddress subnetMask(255, 255, 255, 0);
IPAddress gateway(10, 176, 32, 1);

constexpr uint32_t kDHCPTimeout = 15000;
constexpr uint16_t teensy_udp_port_listening = 8003;
constexpr uint16_t PC_udp_port_listening = 8003;
// Dedicated low-rate port for GetSetParamCommand responses only — matches
// SystemConfig.h's udp_port_param_response_PC_teensy[3] on the PC side.
...
// perturb that path.
constexpr uint16_t PC_udp_port_param_response_listening = 8013;
...
// ----- ODrive objects (right arm: node_id 14-17) -----
ODriveCAN odrv14(wrap_can_intf(ODRV14_CAN), ODRV14_CAN_NODE_ID);
ODriveCAN odrv15(wrap_can_intf(ODRV15_CAN), ODRV15_CAN_NODE_ID);
ODriveCAN odrv16(wrap_can_intf(ODRV16_CAN), ODRV16_CAN_NODE_ID);
ODriveCAN odrv17(wrap_can_intf(ODRV17_CAN), ODRV17_CAN_NODE_ID);

ODriveCAN* odrives[]      = {&odrv14, &odrv15, &odrv16, &odrv17};
ODriveCAN* odrives_can1[] = {&odrv14, &odrv15};
ODriveCAN* odrives_can2[] = {&odrv16, &odrv17};
...
static constexpr float q_min_turns[]   = {ODRV14_Q_MIN_TURNS, ODRV15_Q_MIN_TURNS, ODRV16_Q_MIN_TURNS, ODRV17_Q_MIN_TURNS};
static constexpr float q_max_turns[]   = {ODRV14_Q_MAX_TURNS, ODRV15_Q_MAX_TURNS, ODRV16_Q_MAX_TURNS, ODRV17_Q_MAX_TURNS};
static constexpr float tau_max_nm[]    = {ODRV14_TAU_MAX_NM, ODRV15_TAU_MAX_NM, ODRV16_TAU_MAX_NM, ODRV17_TAU_MAX_NM};
static constexpr float vel_max_turns_s[] = {ODRV14_VEL_MAX_TURNS_S, ODRV15_VEL_MAX_TURNS_S, ODRV16_VEL_MAX_TURNS_S, ODRV17_VEL_MAX_TURNS_S};
...
ODriveUserData odrv14_user_data(ODRV14_CAN_BUS_ID, ODRV14_CAN_ORDER_ID, &ODRV14_CAN);
ODriveUserData odrv15_user_data(ODRV15_CAN_BUS_ID, ODRV15_CAN_ORDER_ID, &ODRV15_CAN);
ODriveUserData odrv16_user_data(ODRV16_CAN_BUS_ID, ODRV16_CAN_ORDER_ID, &ODRV16_CAN);
ODriveUserData odrv17_user_data(ODRV17_CAN_BUS_ID, ODRV17_CAN_ORDER_ID, &ODRV17_CAN);

ODriveUserData* odrives_data[] = {
    &odrv14_user_data, &odrv15_user_data,
    &odrv16_user_data, &odrv17_user_data
};

// Real CAN node_id per odrives[] slot (14,15,16,17) — same values as the
// odrv14..17 variable names this time, unlike Teensy 3's offset scheme, but
// kept as an explicit array (not just i+14) for symmetry with teensy3.ino.
static constexpr int node_ids[] = {ODRV14_CAN_NODE_ID, ODRV15_CAN_NODE_ID, ODRV16_CAN_NODE_ID, ODRV17_CAN_NODE_ID};
...
void setup()
{
    ...
    sys_data_->add(SystemData<N_ODRIVE_CAN8>());  // right arm CAN1: r_shoulder_pitch + r_shoulder_roll
    sys_data_->add(SystemData<N_ODRIVE_CAN9>());  // right arm CAN2: r_shoulder_yaw + r_elbow
    ...
    Serial.println("PC<UDP>Teensy4<CAN>ODrivePro setup is complete.");
}
...
bool setupCAN()
{
    // CAN1: manual mailbox assignment for arm node IDs 14–15
    // CAN frame ID = (node_id << 5) | cmd_id
    //   node 14 encoder (cmd 0x09): 14*32+9  = 0x1C9
    //   node 15 encoder (cmd 0x09): 15*32+9  = 0x1E9
    //   node 14 heartbeat (cmd 0x01): 14*32+1 = 0x1C1
    //   node 15 heartbeat (cmd 0x01): 15*32+1 = 0x1E1
    //   node 14 cmd 0x05: 14*32+5 = 0x1C5
    //   node 15 cmd 0x05: 15*32+5 = 0x1E5
    ...
    uint16_t highFreqIDs[] = { 0x1C9, 0x1E9 };
    int mb = 0;
    for (int i = 0; i < 2; i++) {
    ...
    uint16_t lowFreqIDs[] = { 0x1C1, 0x1E1, 0x1C5, 0x1E5 };
    for (int i = 0; i < 4; i++) {
    ...
    // CAN2: arm node IDs 16-17. Previously used distribute() alone, with
...
void sendUDPPacket()
{
    ...
    if (!udp.send(pc_ip_, PC_udp_port_listening, buffer, sizeof(buffer))) {
        printf("Error sending udp from Teensy4\n");
    } else {
        send_count++;
